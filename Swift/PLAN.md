...
└── Swift/
    ...
    │   └── SpaceTraderUI/               # SwiftUI library target, Apple platforms only
...
| `Src/CmdrStatusEvent.c`                           | `SpaceTraderUI/Screens/CommanderStatusView.swift`            |
| `Src/SystemInfoEvent.c` (docked screen + draw)    | `SpaceTraderUI/Screens/SystemInfoView.swift`                 |
| `Src/Cargo.c` buy-side only                       | `SpaceTraderUI/Screens/BuyCargoView.swift`                   |
...
- [x] **Step 13** iOS app target (`SpaceTraderUI` library in `Package.swift`; originally landed as an `iOSApp` executable, refactored 2026-04-23 to a library wrapped by a host Xcode project — see Progress log) + `ContentView.swift` tab root
...
- Create the thin Xcode host app described in `README.md` (iOS → App
  template, add `Swift/` as a local package dep, link both
  `SpaceTraderCore` + `SpaceTraderUI`, wire `ContentView()` + a
  `@StateObject gameState = SpaceTraderBootstrap.makeInitialGameState()`
  in the app's root view).
- Pick any iOS 16+ iPhone simulator.
...
- [2026-04-23] plan: refactored `Sources/iOSApp` → `Sources/SpaceTraderUI` library. The original SwiftPM `executableTarget` produced an iOS binary with no `Info.plist` / `CFBundleIdentifier`, so the simulator crashed on launch (`__BKSHIDEvent__BUNDLE_IDENTIFIER_FOR_CURRENT_PROCESS_IS_NIL__`). Fixed by shipping the views as a library product (ContentView + the three screens + Bootstrap.makeInitialGameState()) and pushing the `@main App` shell into a thin Xcode iOS-App project the user creates outside `Swift/`. README updated with the one-time setup. All view types are `public` with `public init()`. 125/125 tests still green on Linux. No functional change to `SpaceTraderCore` or the tests.
